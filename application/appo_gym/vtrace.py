...
Refer to: https://github.com/voiler/IMPALA/blob/master/vtrace.py
"""
...
def from_importance_weights(
    ...
    if clip_rho_threshold is not None:
        clip_rho_threshold = torch.tensor(
            clip_rho_threshold, dtype=torch.float32, device=log_rhos.device
        )
    if clip_pg_rho_threshold is not None:
        clip_pg_rho_threshold = torch.tensor(
            clip_pg_rho_threshold, dtype=torch.float32, device=log_rhos.device
        )
    ...
    with torch.no_grad():
        ...
        for i in range(seq_len):
            ...
            for j in range(i, seq_len):
                v_s += (
                    torch.prod(discounts[i:j], dim=0)
                    * torch.prod(cs[i:j], dim=0)
                    * clipped_rhos[j]
