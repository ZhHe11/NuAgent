...
class GoalReachingAnt(gym.Wrapper):
    """Ant maze with goal reaching observation. The observation space \
        comprises two components: 1) observation: raw observation; \
        2) goal: goal observation, the space is set to the same as raw observation.
    """
    ...
    def step(self, action):
        ...
        # TODO(ming): retrieving current position, considering
        #   rename it as `get_goal_state`
        achieved = self.get_xy()
        desired = self.target_goal

        # TODO(ming): computing goal reaching information,
        #   considering to make it be more general
