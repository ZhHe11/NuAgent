...
class AsyncAgent:
    ...
    def run_episode(
        ...
        for step_cnt in counter:
            ...
            value, action, log_prob, entropy, net_state = self.act(
        ...
        return rewards, values, log_probs, entropies, len(rewards)
    ...
    def test(self, args: Namespace, ps_rref, counter, lock, log_dir):
        ...
        for epoch in count():
            ...
            model.eval()
            rewards, _, _, _, episode_len = self.run_episode(args, model)
    ...
    def train(self, args: Namespace, ps_rref, counter, lock, rank, log_dir):
        ...
        for epoch in count():
            model.train()
            rewards, values, log_probs, entropies, episode_len = self.run_episode(
                args, model, counter, lock
            )
