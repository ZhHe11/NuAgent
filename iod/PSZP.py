    ...
    def _evaluate_policy(self, runner, env_name):
        if env_name == 'ant_maze':  
            ...
            if wandb.run is not None:
                ...
                path = '.'
            FD, AR, eval_metrics = PlotMazeTrajWindowDist(runner._env, self.DistWindow, self.target_traj_encoder, self.qf1, self.qf2, self.log_alpha, self.option_policy, self.device, dim_option=self.dim_option, max_path_length=self.max_path_length, path=path)
