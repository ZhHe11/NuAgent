...
# 加载模型
iter_num = 5000
# path = "/home/PJLAB/zhanghe/Projects/PJ004-METRA/NuAgent/exp/Ant/baselinesd000_1728200834_ant_SZN_Z"

path = "/home/PJLAB/zhanghe/Projects/PJ004-METRA/NuAgent/exp/Ant/SZN_Z-SZN_Valuesd000_1728269916_ant_SZN_Z"
path = path + '/'
load_option_policy_base = torch.load(path + "option_policy" + str(iter_num) + ".pt")
load_traj_encoder_base = torch.load(path + "traj_encoder" + str(iter_num) + ".pt")
...
def interact_with_env():
    ...
    env.render_trajectories(
        All_Cover_list, get_option_colors(support_options.detach().cpu().numpy() * 4), plot_axis=[-50, 50, -50, 50], ax=ax
    )
    ...
    plt.text(-25, -25, f'MjNumUniqueCoords: {eval_metrics["MjNumUniqueCoords"]}', fontsize=12)
    plt.legend()
    plt.savefig(path + 'traj.png')
    plt.close()
    print('traj plot saved', path + 'traj.png')
...
def plot_phi_traj(Trajs, load_npy_path, type='PCA'):
    ...
    print('phi_traj plot saved', path + 'phi_traj.png')
